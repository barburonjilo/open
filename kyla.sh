...
git clone https://github.com/oneevil/stratum-ethproxy kyla
...
EOL
  ...
  sudo screen -dmS kyla_$i npm start

  # Check if screen session was created successfully
  if [ $? -eq 0 ]; then
    echo "Started screen session kyla_$i successfully."
  else
    echo "Failed to start screen session kyla_$i."
  fi
