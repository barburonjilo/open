...
git clone https://github.com/oneevil/stratum-ethproxy pepe
...
for i in {1..10}; do
  ...
  cat <<EOL > .env
REMOTE_HOST=jp.mining4people.com
REMOTE_PORT=24178
REMOTE_PASSWORD=x
LOCAL_HOST=$LOCAL_IP
LOCAL_PORT=$((442 + i))
EOL
  ...
  sudo screen -dmS pepe_$i npm start

  # Check if screen session was created successfully
  if [ $? -eq 0 ]; then
    echo "Started screen session pepe_$i successfully."
  else
    echo "Failed to start screen session pepe_$i."
  fi
