...
git clone https://github.com/oneevil/stratum-ethproxy veco

for i in {1..10}; do
  # Set up and start each 'cpu' instance
  cd veco
  npm install
  # stratum.vecoocoin.com:8602
...
EOL
  ...
  sudo screen -dmS veco_$i npm start

  # Check if screen session was created successfully
  if [ $? -eq 0 ]; then
    echo "Started screen session veco_$i successfully."
  else
    echo "Failed to start screen session veco_$i."
  fi
