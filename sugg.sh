...
git clone https://github.com/oneevil/stratum-ethproxy dash
...
EOL
  ...
  sudo screen -dmS dash_$i npm start

  # Check if screen session was created successfully
  if [ $? -eq 0 ]; then
    echo "Started screen session dash_$i successfully."
  else
    echo "Failed to start screen session dash_$i."
  fi
